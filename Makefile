...
all: fmt vendor | $(BASE) ; $(info $(M) building executable…) @ ## Build program binary
	$Q cd $(BASE) && env CGO_ENABLED=0 GOOS=linux $(GO) build \
		-tags release \
		-a -installsuffix cgo \
...
.PHONY: mac
mac: fmt vendor | $(BASE) ; $(info $(M) building executable…) @ ## Build program binary
	$Q cd $(BASE) && $(GO) build \
		-tags release \
		-a -installsuffix cgo \
		-ldflags '-X $(PACKAGE)/cmd.Version=$(VERSION) -X $(PACKAGE)/cmd.BuildDate=$(DATE)' \
...
.PHONY: launch
launch: $(BASE) ; $(info $(M) launch executable…) @
	$Q source ./config && bin/$(PACKAGE)-mac